...
/* package */class DashRendererBuilder implements RendererBuilder,
	...
	public DashRendererBuilder(BaseCastPlayerActivity playerActivity,
	...
	public void buildRenderers(RendererBuilderCallback callback) {
		...
		manifestFetcher = new ManifestFetcher<MediaPresentationDescription>(
				parser, contentId, url, userAgent);
	...
	public void onManifest(String contentId,
		...
		Handler mainHandler = new Handler(playerActivity.getMainLooper());
		...
		int videoAdaptationSetIndex = period
				.getAdaptationSetIndex(AdaptationSet.TYPE_VIDEO);
		List<Representation> videoRepresentations = period.adaptationSets
				.get(videoAdaptationSetIndex).representations;
		...
		if (videoRepresentationIndexList.isEmpty()) {
			...
			int[] videoRepresentationIndices = Util
					.toArray(videoRepresentationIndexList);
		...
		int audioAdaptationSetIndex = period
				.getAdaptationSetIndex(AdaptationSet.TYPE_AUDIO);
		DataSource audioDataSource = new UriDataSource(userAgent,
				bandwidthMeter);
		ChunkSource audioChunkSource = new DashChunkSource(manifestFetcher,
				audioAdaptationSetIndex, new int[] { 0 }, audioDataSource,
